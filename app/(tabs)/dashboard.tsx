...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const req = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/db-data`,
        {
          headers: {
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <View className="flex-1 bg-bgLight dark:bg-bgDark py-5">
          ...
          <BalanceCard account={account?? {balance:0, available_balance:0}} />

          <View className="my-7 px-3">
            <Text className="text-sm text-primary mb-3">Quick Access</Text>
